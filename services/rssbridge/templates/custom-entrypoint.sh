...
while IFS= read -r -d '' file; do
    ...
    case "$file_name" in
    *Bridge.php)    yes | cp "$file" "$APP_DIR/bridges/" ;
                    printf "Custom Bridge %s added.\n" $file_name;;
    *Format.php)    yes | cp "$file" "$APP_DIR/formats/" ;
                    printf "Custom Format %s added.\n" $file_name;;
    config.ini.php) yes | cp "$file" "$APP_DIR/" ;
                    printf "Custom config.ini.php added.\n";;
    whitelist.txt)  yes | cp "$file" "$APP_DIR/" ;
                    printf "Custom whitelist.txt added.\n";;
    DEBUG)          yes | cp "$file" "$APP_DIR/" ;
